...
class MatterTemperatureSensorBattery : public MatterEndPoint {
public:
  MatterTemperatureSensorBattery();
  ~MatterTemperatureSensorBattery();
  ...
  // ---- Battery reporting (Power Source cluster) ----
  // percent: 0–100 (will be converted to Matter half-percent units internally)
  bool setBatteryPercent(uint8_t percent);
  // battery voltage in millivolts (e.g. 3975 for 3.975V)
  bool setBatteryVoltageMv(uint32_t mv);
